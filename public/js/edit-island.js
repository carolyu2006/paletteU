...
function init() {
    if (window.editIslandInitialized) return;
    window.editIslandInitialized = true;

    initMemorySelector();
    initModelSelector();
    initColorSelector();
    initFormValidation();
    initializeFromIslandData();
}

function initializeFromIslandData() {
    const islandData = window.islandData;
    if (!islandData) return;

    // Set color selection
    if (islandData.color) {
        const colorBtn = document.querySelector(`.color-btn[data-color="${islandData.color}"]`);
        if (colorBtn) {
            colorBtn.classList.add('selected');
            const colorInput = document.getElementById('color-input');
            if (colorInput) {
                colorInput.value = islandData.color;
            }
        }
    }
    ...
    if (islandData.model) {
        ...
        if (modelInput) {
            modelInput.value = islandData.model;
            
            // Find and select the model in the modal
            const modelItem = document.querySelector(`.model-item[data-model-name="${islandData.model}"]`);
            if (modelItem) {
                modelItem.setAttribute('data-selected', 'true');
                ...
                if (display) {
                    ...
                    img.src = modelItem.getAttribute('data-model-preview-url');
                    img.alt = modelItem.getAttribute('data-model-display-name');
                    img.className = 'selected-model-image';
                    img.onerror = () => img.style.display = 'none';
                    card.appendChild(img);
                    display.appendChild(card);
                    
                    modelInput.setAttribute('data-model-url', modelItem.getAttribute('data-model-url') || '');
                }
            }
        }
    }

    // Set memories selection
    if (islandData.memories && islandData.memories.length > 0) {
        ...
        if (selectedInputs) {
            ...
            islandData.memories.forEach(memoryId => {
...
function initModelSelector() {
    ...
    if (islandData && islandData.model) {
        selectedModel = models.find(m => m.name === islandData.model) || null;
        if (selectedModel) {
            const modelItem = document.querySelector(`.model-item[data-model-name="${islandData.model}"]`);
            if (modelItem) {
    ...
    // Initialize display with existing model
    if (selectedModel) {
        updateDisplay(selectedModel);
    } else {
        updateDisplay(null);
