...
export default class Model {
	...
	constructor(obj) {
		...
		this.dracoLoader.setDecoderPath('https://www.gstatic.com/draco/version/1.5.6/')
		...
		this.defaultMatcap = obj.replaceURL
			...
			: this.textureLoader.load('/threejs/mat/1.png')
	...
	init() {
		...
		this.loader.load(
			...
			(gltf) => {
				...
				// Only add to scene if the mesh doesn't already have a parent (callback may have added it to a parentGroup)
				if (!this.meshes[`${this.name}`].parent) {
			...
			(error) => {
				console.error(`Error loading model ${this.name} (${this.file}):`, error)
			}
		)
	}
